...
{#snippet accountContent()}
	...
	<div class="account-group account-container">
		...
		<div class="account-item">
			<p>{$_('rsvp.accordion.account_number5')}</p>
			<p>{$_('rsvp.accordion.account_name5')}
				<button class="copy-btn" on:click={() => copyAccount($_('rsvp.accordion.account_number5').split(' ').slice(1).join(' '))}>
					{$_('rsvp.accordion.copy_account')}
				</button>
			</p>
		</div>
